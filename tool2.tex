...
\dacomment{
Like Android Battery, Battery+ estimates the screen energy in real
time. Real time screen energy estimation suffers from the computational
...
We implemented Battery+, by modifying {\small `screenrecord`} command code,
as a 1000 LOC C++ program  which runs via {\small `adb shell`} interface.
Further, we incorporated Battery+ into Pixel 2's Android framework for providing
an interface via settings app.
To account screen energy for individual apps, we modified the Activity manager which
informs the modified Battery Stats of each app running on the screen.
Modified Battery Stats records the app on-screen time and estimates
OLED power draw and energy calculations per app.
In total, the Android Battery+ is implemented with 1200 LOC in
the Android framework and 20 LOC in the Settings app. ADDED}
